            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('customers.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label for="max_daily_minutes" class="block text-sm font-medium text-gray-700">Max Daily Minutes (0 = unlimited)</label>
                            <input type="number" name="max_daily_minutes" id="max_daily_minutes" min="0" value="{{ old('max_daily_minutes') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label for="max_monthly_minutes" class="block text-sm font-medium text-gray-700">Max Monthly Minutes (0 = unlimited)</label>
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('customers.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-md text-sm font-medium">Cancel</a>
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md text-sm font-medium">Create Customer</button>
                    </div>
                </form>
            </div>
        </div>
